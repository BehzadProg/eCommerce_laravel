...
class VendorProductVariantItemDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function ($query) {
            return '<a href="' . route('vendor.variant-item.edit', ['productId' => $query->productVariant->product_id , 'variantItemId' => $query->id]) . '" class="btn btn-outline-primary"><i class="far fa-edit"></i></a>
                   <a href="' . route('vendor.variant-item.destroy', $query->id) . '" class="btn btn-outline-danger delete-item ml-2"><i class="fas fa-trash-alt"></i></a>';
        })
        ...
        ->addColumn('price' , function($query){
            if($query->price == 0){
                return '<i class="badge bg-success">Free</i>';
            }else{
                return $query->price;
            }
        })
        ...
        ->rawColumns(['action', 'status' , 'is_default' , 'price'])
            ->setRowId('id');
    }
