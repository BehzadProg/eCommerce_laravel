...
class FrontendProductController extends Controller
{
    public function productIndex(Request $request)
    {
        if($request->has('category')){
            ...
            $products = Product::where(['category_id' => $category->id , 'status' => 1 , 'is_approved' => 1])->latest()->paginate(12);
        }elseif($request->has('subcategory')){
            $subCategory = SubCategory::where('slug' , $request->subcategory)->firstOrFail();
            $products = Product::where(['sub_category_id' => $subCategory->id , 'status' => 1 , 'is_approved' => 1])->latest()->paginate(12);
        }elseif($request->has('childcategory')){
            $childCategory = ChildCategory::where('slug' , $request->childcategory)->firstOrFail();
            $products = Product::where(['child_category_id' => $childCategory->id , 'status' => 1 , 'is_approved' => 1])->latest()->paginate(12);
        }
