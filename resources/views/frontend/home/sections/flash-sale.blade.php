                    ...
                    <div class="wsus__product_item">
                        ...
                        @if (checkDiscount($item->product))
                            <span class="wsus__minus">-
                                {{ calculateDiscountPercend($item->product->price, $item->product->offer_price) }} %
                            </span>
                        @endif
                        <a class="wsus__pro_link" href="{{ route('product-detail', $item->product->slug) }}">
                            <img src="{{ asset(env('ADMIN_PRODUCT_IMAGE_UPLOAD_PATH') . $item->product->thumb_image) }}"
                                alt="product" class="img-fluid w-100 img_1" />
                            <img src="
                        @if (isset($item->product->productImageGalleries[0]->image)) {{ asset(env('ADMIN_PRODUCT_GALLERY_IMAGE_UPLOAD_PATH') . $item->product->productImageGalleries[0]->image) }}
                        @else
                        {{ asset(env('ADMIN_PRODUCT_IMAGE_UPLOAD_PATH') . $item->product->thumb_image) }} @endif
                        "
                        ...
                        <div class="wsus__product_details">
                            <a class="wsus__category" href="{{route('product.index', ['category' => $item->product->category->slug])}}">{{ $item->product->category->name }} </a>
                            <p class="wsus__pro_rating">

                                @php
                                 $avgRating = ceil($item->product->productReviews->avg('rate'));
                                 @endphp
                                 @for ($i = 1; $i <= 5; $i++)
                                    @if ($i <= $avgRating)
                                    ...
                                    @endif
                                  @endfor
                                <span>({{$item->product->productReviews()->count()}} review)</span>
                            </p>
                            ...
                            <div class="col-xl-6 col-12 col-sm-10 col-md-8 col-lg-6 m-auto display">
                                <div class="wsus__quick_view_img">
                                    ...
                                    <div class="row modal_slider">
                                        ...
                                        @if (count($item->product->productImageGalleries) == 0)
                                            <div class="col-xl-12">
                                                <div class="modal_slider_img">
                                                    <img src="{{ asset(env('ADMIN_PRODUCT_IMAGE_UPLOAD_PATH') . $item->product->thumb_image) }}"
                                                        alt="product" class="img-fluid w-100">
                                        ...
                                        @endif
                                        @foreach ($item->product->productImageGalleries as $imageGallery)
                                            <div class="col-xl-12">
                                                <div class="modal_slider_img">
